...
/*   Updated: 2024/02/19 14:39:22 by gyoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
LocationConfig::LocationConfig()
    : AConfig(), AHttpConfigCore(), modifier(), pattern(), alias(), proxyPass(),
      cgiPath()
{
}

LocationConfig::LocationConfig(const LocationConfig &other)
    : AConfig(other), AHttpConfigCore(other), modifier(other.modifier),
      pattern(other.pattern), alias(other.alias), proxyPass(other.proxyPass),
      cgiPath(other.cgiPath)
{
}

LocationConfig::LocationConfig(const AHttpConfigCore &core)
    : AConfig(), AHttpConfigCore(core), modifier(), pattern(), alias(),
      proxyPass(), cgiPath()
{
}

LocationConfig::LocationConfig(
    const ConfigFile &block,
      ...
      proxyPass(), cgiPath()
{
    ...
    size_t numToken;
    bool hasAlias = false, hasRoot = false, hasProxyPass = false,
         hasCgiPath = false;
...
void LocationConfig::setModifier(const std::string &modifier)
{
