...
/*   Updated: 2024/02/19 14:29:33 by gyoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
HttpConfig::HttpConfig(const HttpConfig &other)
    : AConfig(other), AHttpConfigCore(other), servers(other.servers)
{
}

HttpConfig::HttpConfig(const ConfigFile &block) throw(std::logic_error)
    : AConfig(), AHttpConfigCore(), servers()
{
    ...
    for (size_t i = 0; i < block.getSubBlocks().size(); i++)
    {
        ...
        if (subBlockName == "server")
        {
            ServerConfig newServerConfig =
                ServerConfig(block.getSubBlocks().at(i), *this);
            ...
            for (; it_names != newServerConfig.getNames().end(); ++it_names)
            {
                ...
                for (; it_ports != newServerConfig.getPorts().end(); ++it_ports)
                {
                    ...
                    {
                        if (servers[j].hasName(*it_names) &&
                            servers[j].hasPort(*it_ports))
                        {
                            std::cout << "warning: conflicting server name \"" +
                                             *it_names;
                            std::cout << "\" on 0.0.0.0:" +
                                             util::string::itos(*it_ports) +
                                             ", ignored"
                                      << std::endl;
                            isDuplicated = true;
                        }
