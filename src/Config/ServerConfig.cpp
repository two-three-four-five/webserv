...
/*   Updated: 2024/02/19 14:30:18 by gyoon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
ServerConfig::ServerConfig()
    : AConfig(), AHttpConfigCore(), names(), ports(), locations(3)
{
    ports.insert(80);
}

ServerConfig::ServerConfig(const ServerConfig &other)
    : AConfig(other), AHttpConfigCore(other), names(other.names),
      ports(other.ports), locations(other.locations)
{
}

ServerConfig::ServerConfig(const ConfigFile &block,
                           const AHttpConfigCore &core) throw(std::logic_error)
    : AConfig(), AHttpConfigCore(core), names(), ports(), locations(3)
{
    ...
    for (size_t i = 0; i < block.getSubBlocks().size(); i++)
    {
        ...
        {
            LocationConfig conf =
                LocationConfig(block.getSubBlocks().at(i), *this);
            size_t mIdx = 0;
            if (conf.getModifier() == "=")
                mIdx = 0;
...
const std::vector<std::vector<LocationConfig> > &ServerConfig::getLocations()
    const
{
    return locations;
}

bool ServerConfig::hasName(const std::string &name) const
{
    return names.count(name) > 0;
}

bool ServerConfig::hasPort(const unsigned short port) const
{
...
std::ostream &operator<<(std::ostream &os, const ServerConfig &conf)
{
    ...
    os << "\tnames: ";
    for (std::set<std::string>::const_iterator it = conf.getNames().begin();
         it != conf.getNames().end(); ++it)
        os << *it << " ";
    os << std::endl;

    os << "\tports: ";
    for (std::set<unsigned short>::const_iterator it = conf.getPorts().begin();
         it != conf.getPorts().end(); ++it)
