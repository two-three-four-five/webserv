...
Response::Response(const Response &other)
    : responseState(other.responseState), statusLine(other.statusLine),
      headers(other.headers), body(other.body), writtenBytes(0),
      responseBytes(0)
{
}
                        ...
                        const std::string &targetResource)
{
    ...
    if (typeIt != typeMap.end())
    {
        ...
        if (typeIt->second == "text/html" || typeIt->second == "text/css" ||
            typeIt->second == "text/xml")
...
Response::ResponseState Response::getResponseState() const
{
...
void Response::setResponseBuffer()
{
    ...
    for (HeaderMultiMap::iterator it = headers.begin(); it != headers.end();
         it++)
...
void Response::addToHeaders(std::string key, std::string value)
{
...
int Response::getWrittenBytes() const { return writtenBytes; }

int Response::getResponseBytes() const { return responseBytes; }

void Response::send(int fd)
{
    const char *wrBuffer = responseBuffer.c_str();
    size_t bytesToWrite =
        std::min(responseBytes - writtenBytes, (unsigned long)BUFFER_SIZE);
