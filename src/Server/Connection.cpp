...
Connection::Connection(const Connection &other)
    : request(other.request), response(other.response), socket(other.socket),
      port(other.port), statusCode(other.statusCode),
      startTime(other.startTime), targetServer(other.targetServer),
      targetLocationConfig(other.targetLocationConfig),
      targetResource(other.targetResource), end(other.end)
{
}
...
void Connection::readRequest(int fd)
{
    ...
    if (request.getParseStatus() >= Body)
    {
        ...
        {
            targetServer =
                Webserv::getInstance().findTargetServer(port, request);
            targetResource = configureTargetResource(
                request.getRequestTarget().getTargetURI());
        }
        if (targetLocationConfig.getClientMaxBodySize() <
            getRequest().getBodyLength())
        {
            statusCode = 413;
        }
    }
...
std::string Connection::getTargetResource(std::string &requestTarget)
{
    ...
    if (!selectedAlias.empty())
    {
        if (selectedPattern.back() == '/')
            tempTargetResource =
                selectedAlias +
                requestTarget.substr(selectedPattern.length() - 1);
        else
            tempTargetResource =
                selectedAlias + requestTarget.substr(selectedPattern.length());
    }
    else
    ...
    if (tempTargetResource.back() == '/')
    {
        ...
        else
        {
            const std::set<std::string> &indexes =
                targetLocationConfig.getIndexes();
...
{
    // =
    const std::vector<LocationConfig> &equalLocations =
        targetServer->getServerConfig().getLocations().at(0);
    std::vector<LocationConfig>::const_iterator selectedIt =
        equalLocations.end();

    for (std::vector<LocationConfig>::const_iterator it =
             equalLocations.begin();
         it != equalLocations.end(); it++)
    {
    ...
    const std::vector<LocationConfig> &locations =
        targetServer->getServerConfig().getLocations().at(2);
...
{
    // $
    const std::vector<LocationConfig> &endLocations =
        targetServer->getServerConfig().getLocations().at(1);

    for (std::vector<LocationConfig>::const_iterator it = endLocations.begin();
         it != endLocations.end(); it++)
    {
        const std::string &requestTarget =
            request.getRequestTarget().getTargetURI();
        const std::string &pattern = it->getPattern();
        if (requestTarget.rfind(pattern) ==
            requestTarget.length() - pattern.length())
        {
    ...
    else
    {
        ...
        if (method == "GET")
        {
            ...
            if (targetFile.isDirectory() &&
                targetLocationConfig.getAutoIndex() &&
                targetResource.back() == '/')
                buildDirectoryResponse();
            else if (targetFile.isDirectory())
                build301Response(
                    "http://" + request.getHeaders().find("host")->second +
                    request.getRequestTarget().getTargetURI() + "/");
            else if (targetFile.isReadable() ||
                     targetLocationConfig.getProxyPass().length() != 0)
                buildGetResponse();
            else if (!targetFile.isReadable())
...
void Connection::buildErrorResponse(int statusCode)
{
    std::string startLine =
        "HTTP/1.1 " + Webserv::getInstance().getStatusMessage(statusCode);
    response.setStatusLine(startLine);
    // best matching LocationConfig == targetLocationConfig
    std::map<int, std::string> errorPages =
        targetLocationConfig.getErrorPages();
    // found error page
...
void Connection::buildCGIResponse(const std::string &scriptPath)
{
    ...
    if (cgiPID == 0)
    {
        char *argv[] = {(char *)scriptPath.c_str(),
                        (char *)targetResource.c_str(), NULL};
        char **envp = makeEnvp();
        close(outward_fd[0]);
        close(inward_fd[1]);
...
char **Connection::makeEnvp()
{
    ...
    envVec.push_back("QUERY_STRING=" +
                     request.getRequestTarget().getQueryString());
...
const LocationConfig &Connection::getTargetLocationConfig() const
{
    return targetLocationConfig;
}
