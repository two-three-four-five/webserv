...
Request::Request()
    : parseStatus(Created), connectionClose(false), contentLength(-1),
      bodyLength(0), body(), chunkSize(0), buffer(), isEnd(false)
{
    oss.str("");
    oss.clear();
}
...
Request::Request(const Request &other)
    : parseStatus(other.parseStatus), method(other.method),
      requestTarget(other.requestTarget), headers(other.headers),
      boundary(other.boundary), connectionClose(other.connectionClose),
      contentLength(other.contentLength), bodyLength(other.bodyLength),
      body(other.body), parseBody(other.parseBody), chunkSize(other.chunkSize),
      buffer(other.buffer)
{
    oss.str("");
    oss.clear();
...
void Request::setBodyParseFunction()
{
    ...
    // Boundary
    std::map<std::string, std::string>::iterator contentTypeIt =
        headers.find("content-type");
    if (contentTypeIt != headers.end())
    {
        std::vector<std::string> contentType =
            parseContentType(contentTypeIt->second);
        std::vector<std::string>::iterator it =
            std::find(contentType.begin(), contentType.end(), "boundary");
    ...
    // Transfer-Encoding
    std::map<std::string, std::string>::iterator transferEncodingIt =
...
int Request::checkHeaderField()
{
    ...
    if (transferEncodingIt != headers.end())
    {
        std::vector<std::string> transferEncoding =
            parseTransferEncoding(transferEncodingIt->second);
        std::vector<std::string>::iterator chunkedIt = std::find(
            transferEncoding.begin(), transferEncoding.end(), "chunked");
        if (chunkedIt == transferEncoding.end() ||
            chunkedIt + 1 != transferEncoding.end() ||
    ...
    if (method == "POST" && transferEncodingIt == headers.end() &&
        contentLengthIt == headers.end())
    {
        parseStatus = End;
        return 411;
    ...
    if (contentLengthIt != headers.end() &&
        !util::string::isAllDigit(contentLengthIt->second))
    {
        parseStatus = End;
        return 400;
    }
    ...
    if (hostIt == headers.end() ||
        hostIt->second.find(",") != std::string::npos)
    {
        parseStatus = End;
        return 400;
    }
...
void Request::removeChunkField()
{
    ...
    for (std::vector<std::string>::iterator it = teVec.begin();
         it < teVec.end(); it++)
    {
        if (*it == "chunked")
            continue;
...
std::ostream &Hafserv::operator<<(std::ostream &os,
                                  const Hafserv::Request &request)
{
    os << request.getMethod() << " " << request.getRequestTarget()
       << " HTTP/1.1\r\n";
