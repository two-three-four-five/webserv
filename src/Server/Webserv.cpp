...
void Webserv::addServer(Server *server)
{
    ...
    for (std::set<unsigned short>::const_iterator it =
             server->getPorts().begin();
         it != server->getPorts().end(); it++)
        openPort(*it);
}
...
void Webserv::openPort(unsigned short port)
{
    if (portToServSock.find(port) == portToServSock.end())
    {
        ...
        if (bind(socketFd, (struct sockaddr *)&serv_adr, sizeof(serv_adr)) ==
            -1)
        {
        ...
        for (int i = 0; i < events; i++)
        {
            ...
            try
            {
                ...
                else if (inClientSocks(eventFd))
                {
                    ...
                    {
                        Connection &conn = findConnectionByFd(eventFd);
                        if (conn.getResponse().getResponseState() ==
                            Response::Ready)
                            conn.sendResponse();
                        else if (conn.getResponse().getResponseState() ==
                                 Response::Sending)
                            conn.sendResponse();
                        if (conn.getResponse().getResponseState() ==
                            Response::End)
                        {
                            if (conn.getStatusCode() ||
                                conn.getRequest().getConnectionClose())
                            {
                                disconnectClient(eventFd);
                            }
                            else
                ...
                {
                    Connection &conn = findConnectionByFd(
                        cgiFdToConnectionFd.find(eventFd)->second);
...
void Webserv::connectClient(int serv_sock)
{
    ...
    Connections.insert(std::make_pair(
        clnt_sock, Connection(clnt_sock, sockToPort.find(clnt_sock)->second)));
...
void Webserv::disconnectClient(int socketfd)
{
    ...
    std::map<int, unsigned short>::iterator socketToPortIt =
        sockToPort.find(socketfd);
...
Server *Webserv::findTargetServer(unsigned short port, const Request &request)
{
         ...
         it != servers.end(); it++)
    {
        ...
        std::set<std::string>::const_iterator findNameIt =
            serverNames.find(hostName);
...
std::string Webserv::getStatusMessage(int statusCode)
{
    if (statusCodeMap.find(statusCode) == statusCodeMap.end())
        ...
        return util::string::itos(statusCode) + " " +
               statusCodeMap.find(statusCode)->second;
}
...
void Webserv::checkTimeout()
{
         ...
         it != Connections.end(); it++)
    {
        ...
        {
            const LocationConfig targetConfig =
                it->second.getTargetLocationConfig();
            int bodyTimeout = 60;
    ...
    for (std::vector<int>::iterator it = timeoutSockets.begin();
         it != timeoutSockets.end(); it++)
    {
        disconnectClient(*it);
    }
}

bool Webserv::inServSocks(int fd)
{
    return servSockToPort.find(fd) != servSockToPort.end();
}

bool Webserv::inClientSocks(int fd)
{
    return sockToPort.find(fd) != sockToPort.end();
}

bool Webserv::inCGISocks(int fd)
{
    return cgiFdToConnectionFd.find(fd) != cgiFdToConnectionFd.end();
}

void Webserv::closeServSocks()
{
    for (std::map<int, unsigned short>::iterator it = servSockToPort.begin();
         it != servSockToPort.end(); it++)
    {
        close(it->first);
    }
}

const std::map<int, std::string> &Hafserv::Webserv::getStatusCodeMap() const
{
